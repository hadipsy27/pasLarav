    ...
    <div class="row">
        <div class="col-8">
            ...
            <form method="post" action="/students/{{ $student->id }}">
                @method('patch')  {{-- bisa menggunakan @method('put') sama aja --}}
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('nrp') is-invalid @enderror" id="nrp" placeholder="NIM" name="nrp" value="{{ $student->nrp }}">
                    @error('nrp')<div class="invalid-feedback">{{$message}}</div>@enderror
                </div>
                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" placeholder="example@email.com" name="email" value="{{ $student->email }}">
